/**
 * Rebuild multi-page site from the last committed demo.html (clean UTF-8).
 * All user-facing punctuation is ASCII so PowerShell/Windows can't mojibake it.
 */
...
const originalPath = path.join(root, 'public', '_original-demo.html');

if (!fs.existsSync(originalPath)) {
  const buf = execSync('git show HEAD:public/demo.html', { maxBuffer: 20e6 });
  fs.writeFileSync(originalPath, buf);
}

/** Strip fancy punctuation that gets corrupted on some Windows pipelines. */
function ascii(s) {
  return s
    .replace(/\uFEFF/g, '')
    .replace(/[\u2014\u2013\u2212]/g, '-')
...
function writeUtf8(file, text) {
  fs.writeFileSync(file, Buffer.from(ascii(text), 'utf8'));
}

const raw = ascii(fs.readFileSync(originalPath, 'utf8'));
const cssMatch = raw.match(/<style>([\s\S]*?)<\/style>/);
const scriptMatch = raw.match(/<script>([\s\S]*?)<\/script>/);
if (!cssMatch || !scriptMatch) throw new Error('could not extract css/script from original');
...
<title>${title}</title>
<meta name="description" content="Paid, provenance-checked reads of standardized onchain data. x402 on Hedera, The Graph, onchain split with slashable holdback." />
...
<footer>
  <div class="wrap">
    ...
    <p class="byline">Made by <a href="https://github.com/KartikDehru" target="_blank" rel="noopener">Kartik Dehru</a> | <a href="${DOCS}" target="_blank" rel="noopener">GitHub docs</a> | MIT | ETHOnline 2026</p>
  </div>
</footer>
${scripts}
</body>
  ...
  <section id="how">
    <div class="wrap">
      ...
      <div class="steps-grid">
        <div class="st"><div class="top"><span class="no">01</span><span class="role">paywall</span></div><h4>Challenge</h4><p>Unpaid read returns <code>402</code> with x402 requirements. No key, no signup.</p></div>
        ...
        <div class="st"><div class="top"><span class="no">05</span><span class="role">money</span></div><h4>Settle or refuse</h4><p>Pass settles in HBAR. Fail returns 409, unsettled.</p></div>
        <div class="st"><div class="top"><span class="no">06</span><span class="role">evidence</span></div><h4>Sign a receipt</h4><p>Answer, sources, and blocks - hashed and signed.</p></div>
        <div class="st"><div class="top"><span class="no">07</span><span class="role">liability</span></div><h4>Split and hold back</h4><p>Sources get paid; a slashable slice stays unvested.</p></div>
      </div>
    </div>
  </section>
    ...
    <div class="wrap">
      ...
      <div class="cards">
        ...
        <div class="card"><div class="top"><h4>Hedera</h4><span class="role">the money</span></div><ul>
          <li>x402 over HBAR via Blocky402; <code>SourcePayouts</code> for split and holdback.</li>
  ...
  <section>
    <div class="wrap">
      ...
      <ul class="limits">
        ...
        <li><b>Disputes are evidence-backed, not fully trustless</b><span>Graph re-derive offchain; onchain open plus arbiter or deadline uphold.</span></li>
        <li><b>Testnet scale</b><span>Hedera testnet HBAR and a demo liability pool - not insurance.</span></li>
        <li><b>Resale float is operator-funded</b><span>Bazantic USDC and our HBAR rails do not bridge.</span></li>
      </ul>
    </div>
  </section>
</main>
...
const demoStart = raw.indexOf('<section id="demo">');
const exploreMarker = raw.indexOf('<!-- ────────────────────────────── EXPLORER');
const exploreAlt = raw.indexOf('<section id="explore">');
const demoEnd = exploreMarker !== -1 ? exploreMarker : exploreAlt;
if (demoStart < 0 || demoEnd < 0) throw new Error('demo section markers missing');
const demoSection = raw.slice(demoStart, demoEnd);
...
const stackMarker = raw.indexOf('<!-- ────────────────────────────── STACK');
const stackAlt = raw.indexOf('<section id="stack">');
const exploreEnd = stackMarker !== -1 ? stackMarker : stackAlt;
if (exploreStart < 0 || exploreEnd < 0) throw new Error('explore section markers missing');
const exploreSection = raw.slice(exploreStart, exploreEnd);
...
await sm.health()</pre>
      ...
      <p class="lede" style="margin-top:24px">Full docs: <a href="${DOCS}/tree/main/sdk" target="_blank" rel="noopener">${DOCS}/tree/main/sdk</a></p>
    </div>
  </section>
</main>
`;
...
// JS from original script
let script = ascii(scriptMatch[1]);
script = script
...
writeUtf8(path.join(root, 'public', 'js', 'demo.js'), `'use strict';\n${script}`);
writeUtf8(
  path.join(root, 'public', 'js', 'explore.js'),
  `'use strict';\n${script.replace(
...
console.log('rebuilt clean pages');
for (const f of ['index.html', 'demo.html', 'explore.html', 'sdk.html']) {
  const t = fs.readFileSync(path.join(root, 'public', f), 'utf8');
  const bad = /â€|Â.|<>\/a>|<\/\s*\/a>/.test(t);
  console.log(f, 'bytes', Buffer.byteLength(t), 'broken?', bad);
}
